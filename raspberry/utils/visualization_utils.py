...
there_is_an_object = False;
center_object_x = [0 for i in range(100)]
center_object_y = [0 for i in range(100)]
area_object = [0 for i in range(100)]
...
def user_orientation(
  ...
  Returns:
    uint8 numpy array with shape (img_height, img_width, 3) with overlaid boxes.
  """
  global there_is_an_object
  global center_object_x
  global center_object_y
  global area_object
  # Create a display string (and color) for every box location, group any boxes
  # that correspond to the same location.
  box_to_display_str_map = collections.defaultdict(list)
  box_to_display_str_map2 = collections.defaultdict(list)
  box_to_display_str_map_compare = collections.defaultdict(list)
  box_to_color_map = collections.defaultdict(str)
  box_to_instance_masks_map = {}
  box_to_instance_boundaries_map = {}
  box_to_keypoints_map = collections.defaultdict(list)
        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in six.viewkeys(category_index):
              ...
              class_name = 'N/A'
            display_str = str(class_name)
            display_str2 = str(class_name)
        if not skip_scores:
          if not display_str:
            display_str = '{}%'.format(int(100*scores[i]))
          else:
            display_str = '{}: {}%'.format(display_str, int(100*scores[i]))
            display_str2 = '{}'.format(display_str2)
        if not skip_track_ids and track_ids is not None:
          if not display_str:
            display_str = 'ID {}'.format(track_ids[i])
            display_str2 = 'ID {}'.format(track_ids[i])
          else:
            display_str = '{}: ID {}'.format(display_str, track_ids[i])
            display_str2 = '{}'.format(display_str2)
        box_to_display_str_map[box].append(display_str)
        box_to_display_str_map2[box].append(display_str2)
        box_to_display_str_map_compare[box].append('person') #in order to check if an object is a human
        if agnostic_mode:
          box_to_color_map[box] = 'DarkOrange'
        elif track_ids is not None:
          prime_multipler = _get_multiplier_for_color_randomness()
  ...
  for box, color in box_to_color_map.items():
    ...
    #check if there is any object in the image that is a human
    if box_to_display_str_map2[box] == box_to_display_str_map_compare[box]:
        is_human = True;
        (left, right, top, bottom) = (xmin * im_width, xmax * im_width,
                                  ymin * im_height, ymax * im_height)
        
        area_new = (bottom-top)*(right-left)# If tthere is more than a human, analyze which one is with bigger pixel area, which means closer to the camera, nearer.
        #print("human")
        ...
        if area_new > area_old:
            ...
            angle = (0.09375*center_x)-30
    else:
        (left, right, top, bottom) = (xmin * im_width, xmax * im_width,
                                  ymin * im_height, ymax * im_height)
        #print(i)
        center_object_x[i] = round(left + ((right-left)/2))
        center_object_y[i] = round(bottom + ((top-bottom)/2))
        area_object[i] = (bottom-top)*(right-left)
        i = i+1;
        
  for a in range(len(center_object_x)):# from the box of the human, obtain the human's angle from the camera center
      if ((center_object_x[a] > round(center_x-50)) and (center_object_x[a] < round(center_x+50))):
          if (center_object_y[a]< center_y ):
              if (area_object[i]>(area_old*0.3)):
                  there_is_an_object = True
